	...
	*/
	...
	{  
		...
		{ 
			...
			update_user_login_status($curr_user_id, 'online');
			
			if( !empty($loginType) && ($loginType == 'third-party-authorization') )
			{
				ItemModel::add_item( array('category'=>'third-party-user-login', 'auth-provider'=>$authProvider, 'user-id'=>$curr_user_id) );
			}
		}
	...
	{   
		...
		{
			$message = get_mail_message('password-recovery-mail-message', array(
