...
class UserLogin
{
	...
	*/ 
	public static function do_user_login($form_data=array())
	{
		...
		/** 
		* if a login landing page has been specified using UserManager::set_login_landing_page, the user is redirected there
		* else, they are taken to the page they requested, from which they are logging in
		*/
		$login_success_page = $login_landing_page; // ? $login_landing_page : UserManager::get_login_landing_page();
		...
		UserManager::unset_login_landing_page();
	...
	public static function login($user_login, $pass, $opts = array() ){
		...
		//$user_login = UserManager::get_user_data($user_id, 'login'); 
	...
	private static function register_login($userid, $opts = array() )
	{
		...
		$sql = "INSERT INTO ". UserManager::get_tables_prefix(). "user_logins SET ".
           "user_id    = ".  DataSanitizer::sanitize_data_for_db_query($userid). ", ".
           "ip_address = '". DataSanitizer::sanitize_data_for_db_query($ip_address). "', ".
           "login_page = '". DataSanitizer::sanitize_data_for_db_query($loginpage). "', ".
           "login_type = '". DataSanitizer::sanitize_data_for_db_query($logintype). "', ".
	       "login_time =  NOW()";
		...
		//return ( (!$query) ? false : @mysql_insert_id() ); //return the login id of the current login or false on failure registration of the login 
	}
