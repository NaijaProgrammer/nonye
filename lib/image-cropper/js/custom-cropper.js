function activateImageResizer(opts)
{
	...
	*/
	...
	{  
		...
		{
			//this gives error : jquery cannot call methods on dialog prior to initialization, and no such dialog method 'instance'
			//when we use a higher version of jquery than the one that comes with the plugin
			//usually because the site already has its own jquery version, which we test for (in php) and don't load our own if the site already has
			/*if( (typeof $( "#" + uniqueIDPrefix + "uploaded" ).dialog( "instance" ) === 'object')  )
			{
				$( "#" + uniqueIDPrefix + "uploaded" ).dialog( "instance" ).close();
			}*/
			
			//so, we use this instead. credits: http://stackoverflow.com/a/20083720/1743192
			$( "#" + uniqueIDPrefix + "uploaded" ).closest('.ui-dialog-content').dialog('close');
