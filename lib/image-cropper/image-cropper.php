...
class ImageCropper
{
	...
	* action_page string
	* post_data array of extra data to be sent along with the request in key/value pairs
	...
	*/
	public static function get_image_cropper($opts = array())
	{
		...
		'(function(){'.
		 ...
		 '{'.
			'var s = document.getCreateElement("script");'.
			's.src="'. $rel_url. '/js/jquery.min.js";'.
			'document.getElementsByTagNamge("head")[0].appendChild(s)'.
		 '}'.
		'})()'.
		'</script>'.
		...
		'{'. NL.
		    //'if(typeof jQuery( "#'.  $unique_id_prefix. 'uploaded" ).dialog( "instance" ) === "object")'. NL. //see notes in custom-cropper.js file for explanations of this decision
			    //'{'. NL.
				    //'jQuery( "#'. $unique_id_prefix. 'uploaded" ).dialog( "instance" ).close();'. NL.
					'jQuery( "#'. $unique_id_prefix. 'uploaded" ).closest(".ui-dialog-content").dialog("close");'. NL.
			//'}'. NL.
		'}'.
		'</script>';
