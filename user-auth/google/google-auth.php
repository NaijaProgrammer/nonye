...
$auth_service  = new AuthService(array(
	...
	'access_token_session_name' => 'google_access_token',
	'permissions'               => get_external_app_data($auth_provider)['permissions'],
	'auth_callback_url'         => SITE_URL. '/user-auth/google',
));

if( isset($_GET['state']) && isset($_GET['code']) && ($_GET['state'] == 'auth-provider=google') )
{
	//successful authentication/authorization
	//the $_GET['code'] is used internally by the $google_auth object (in the call to get_user_data())
	
	$user_data   = $auth_service->get_user_data(); //var_dump($user_data); exit;
...
header("Location: ". $auth_service->get_auth_url($state_value='auth-provider=google')); 
