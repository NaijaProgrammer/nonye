...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if($action == 'delete_forum')
	{
		...
		{
			if( ForumModel::get_forum_instance($forum_id)->delete() )
			{
				$return_data = array('success'=>true, 'message'=>'Forum deleted successfully');
			}
			else
			{
...
<?php $forums_len = count( ForumModel::get_forums() ); ?>
...
$forums     = ForumModel::get_forums();
$forums_len = count($forums);

?>
<?php
$page_instance = AdminPage::get_instance(array());
$page_instance->load_header('', array('page_title'=>'View Available Forums'));
$page_instance->load_nav();
?>
<div class="container">
 <?php echo do_page_heading('Available Forums'); ?>
 ...
 <div class="inline-block main-content">
  ...
  <table class="table table-bordered table-hover table-responsive">
   ...
   <tbody>
    <?php for($i = 0; $i < $forums_len; $i++): ?>
    <?php $serial_no = $i+1; ?>
    <?php $curr_forum = ForumModel::get_forum_instance( $forums[$i] ); ?>
	 <?php $row_class = ( ($serial_no % 2) ? 'odd-row' : 'even-row' ); ?>
	<tr id="forum-<?php echo $curr_forum->get('id'); ?>-row" class="<?php echo $row_class; ?>">
     <td class="text-centered"><?php echo $serial_no; ?></td>
     <td class="text-centered"><?php echo $curr_forum->get('name'); ?></td>
     <td class="text-centered"><?php echo $curr_forum->get('description'); ?></td>
     <td class="text-centered"><?php echo $curr_forum->get_posts_count(); ?></td>
     <td class="text-centered">
	  <ul class="list-inline">
	  <li>
	   <form>
	   <select name="action" class="form-control inline-block bg-no-repeat" style="width:180px; background-position:140px 8px;"
			onchange="applyAction(this, '<?php echo $curr_forum->get('id'); ?>', '<?php echo urlencode($curr_forum->get('name')); ?>')">
...
<script>
function applyAction(eventTarget, forumID, forumName)
{
	...
	switch(action)
	{
        ...
        case "delete"    : if( confirm('Are you sure you want to delete the ' + forumName + ' forum?') ) { deleteForum(forumID, forumName, eventTarget); }
	...
	{  
		location.href= '<?php echo ADMIN_URL; ?>/?dir=forums&page=' + page + '&forum=' + forumID;
	}
	
	function deleteForum(forumID, forumName, evtTarget)
	{
		disable(evtTarget);
		...
		Site.Util.runAjax({
			...
			'requestURL'         : '',
			'timeoutAfter'       : 30,
			'requestData'        : 'action=delete_forum&forum_id=' + forumID,
