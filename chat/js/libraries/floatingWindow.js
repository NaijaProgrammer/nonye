 ...
 */
  init : function(configObj){
   ...
   configObj = configObj || {}; 
   configObj.attributes = configObj.attributes || {};
   configObj.options = configObj.options || {};
   configObj.styleOptions = configObj.styleOptions || {};

   configObj.resizeCallbacks = configObj.resizeCallbacks || [];
   configObj.minimizeCallbacks = configObj.minimizeCallbacks || [];
   configObj.maximizeCallbacks = configObj.maximizeCallbacks || [];
   configObj.closeCallbacks = configObj.closeCallbacks || [];
   configObj.showCallbacks = configObj.showCallbacks || [];
   configObj.focusCallbacks = configObj.focusCallbacks || [];
   ...
   this.element = this.create();
      ...
      if(!this.element.id){
       this.element.id = "floatingWin_" + new Date().getTime();
      }  
   }, //end init()
   ...
   */
   ...
   addToDOM : function(parentElement){ //parentElement is not working yet, TO DO: make it work
   ...
   maximize : function(winObject){
      ...
      if(!winObject.widthMaximized && !winObject.heightMaximized){      
       var bWidth = parseInt(Browser.Size.width());
       var bHeight = parseInt(Browser.Size.height()); 
   ...
   */
   setPosition : function (configObj) {
    ...
    var windowWidth = Browser.Size.width(); 
    var windowHeight = Browser.Size.height();      
    var winHeight = getElemSize(this.element).height; 
    var winWidth = getElemSize(this.element).width; 
