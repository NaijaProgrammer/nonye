...
function registerChatWindowState(chatWin, windowType, receiverId, registerType){
  ...
  var room = roomToMoveTo;
   ...
   if(uid){
    var userDetails = '';
    var userClosureObject = {'userClosureName' : 'retrieveUserData'};
    getUserData(uid, userClosureObject); //defined in buddiesHandler.js
    setTimeout(function(){userDetails = userClosureObject.userClosureName();}, 300);
   } 

   if(receiverId){
    var receiverDetails = '';
    var receiverClosureObject = {'receiverClosureName' : 'retrieveReceiverData'};
    getUserData(receiverId, receiverClosureObject);
    setTimeout(function(){receiverDetails = receiverClosureObject.receiverClosureName();}, 300);
   }   
   ...
   setTimeout(
    function(){ 
     ...
     receiverDetails = eval( '(' + receiverDetails + ')' );
     starterName = userDetails.name;
       ...
       if(usersDetails.length > 0){
          for(var x in usersDetails){
             if(usersDetails[x]['userId'] == uid){
              ...
              starterName = starterData.firstName;
             }
