...
function addRoom(requestType, roomId, roomName){
   ...
   '<a href="#" id="IM_' +  roomName + '" onclick="loadRoom(\'' + roomName + '\'); swapContent(\'IMRoomHolder\', \'IM_' + roomName + '\'); swapContent(\'roomHolder\', \'IMRoomHolder\'); hideElement(\'IMRoomsList\'); return false">' + roomName + '</a>';
   }
   else if(requestType == 'toChatRoom'){
    objectify('roomsList').innerHTML += '' + 
   '<a href="#" id="' +  roomName + '" onclick="loadRoom(\'' + roomName + '\'); swapContent(\'roomHolder\', \'' + roomName + '\'); swapContent(\'IMRoomHolder\', \'roomHolder\'); hideElement(\'roomsList\'); return false">' + roomName + '</a>';
   }
}
 ...
 * in the onRoomJoinCall() function later below fails the first time the room is loaded
 ...
 * onRoomJoinCall() function defined later below
...
function sendMessage(receiverId, requestType)
{
 ...
 var roomTo = 'none';
   ...
   if( trim(textBox.value) != "" )
   {
     ...
     textBox.value = "";  
     errorDiv.innerHTML = ""; 
     hideElement(errorDiv, true);
     textBox.focus();
   }
   else{
    errorManager.displayError({'Error' : 'you cannot send an empty message'}, errorDiv, true);
    showElement(errorDiv, 'display');
       setTimeout(function(){errorDiv.innerHTML = ''; hideElement(errorDiv, true);}, 5000);
   }
}
...
function readServerResponse(requestType,xhr)
{
 ...
 var response = '';
  ...
  for(var j in response.messages){
    ...
    */ 
    //if(((message.indexOf("joined the " + room + " room") != -1) && (sender_id == uid))||((message.indexOf("you are already in this room") != -1) && (sender_id <= 0))){
   ...
   if(users != -1){
    ...
    var uResponse = eval( '(' + uReply + ')' );
      ...
      for(var i in uResponse.users){
       ...
       formatUserDisplay(userName, userId); //defined in (core)chatHandler.js
       //addRoomUser(userId, userName);     //alternate way to add a new room user to the room users' list, also defined in (core)chatHandler.js
      }
...
function updateReadStatus(messageId){
 ...
 makeXHRRequest(serverURL, xhr, function(){handleUpdateReadStatus(xhr)}, "POST", params, true);
}

function handleUpdateReadStatus(xhr){
   if( (xhr.readyState == 4) && (xhr.status == 200) ){
...
function removeRoomUser(userId, userName){
 ...
 var usersLen = currDisplayedUsers.length;
   if(typeof(user) !== 'undefined'){
    user.parentNode.removeChild(user);
      for(var i = 0; i < usersLen; i++){
         if(currDisplayedUsers[i] == userId){
          currDisplayedUsers.splice(i, 1);
          return;
         }
      }
   }
